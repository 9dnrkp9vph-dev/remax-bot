...
var _ckLast='', _cuLast='';
function cookieMap(){ var m={}; try{ String(document.cookie||'').split(';').forEach(function(c){ c=c.trim(); var i=c.indexOf('='); if(i>0) m[c.slice(0,i)]=c.slice(i+1); }); }catch(e){} return m; }
function trailPush(entry){ try{ var a=JSON.parse(sessionStorage.getItem('ysTrail')||'[]')||[]; entry.t=new Date().toLocaleTimeString('he-IL'); a.push(entry); sessionStorage.setItem('ysTrail',JSON.stringify(a.slice(-25))); }catch(e){} }
function trailTick(){
  try{
    var ck=String(document.cookie||'');
    if(ck!==_ckLast){ if(_ckLast){ var a=cookieMap(), b={}; _ckLast.split(';').forEach(function(c){ c=c.trim(); var i=c.indexOf('='); if(i>0) b[c.slice(0,i)]=c.slice(i+1); }); var ch=[]; Object.keys(a).concat(Object.keys(b)).forEach(function(k){ if(a[k]!==b[k] && ch.indexOf(k)<0 && !/^(_ga|_hj|ab\.storage|ttcsid|__rtbh|mp_|ph_|dicbo|_gcl|uzmx|__uzm)/.test(k)) ch.push(k); }); if(ch.length) trailPush({kind:'cookie',changed:ch.map(function(k){ return k+': '+String(b[k]||'').slice(0,14)+'… → '+String(a[k]||'').slice(0,14)+'…'; })}); } _ckLast=ck; }
    var cu=''; try{ cu=String(localStorage.getItem('currentYad2PlusUser')||''); }catch(e){}
    if(cu!==_cuLast){ if(_cuLast){ trailPush({kind:'currentYad2PlusUser',from:_cuLast.slice(0,160),to:cu.slice(0,160)}); } _cuLast=cu; }
  }catch(e){}
}
try{ trailTick(); setInterval(trailTick,700); window.addEventListener('beforeunload',function(){ trailPush({kind:'unload',href:String(location.href).slice(0,120)}); }); }catch(e){}
function trailGet(){ try{ return JSON.parse(sessionStorage.getItem('ysTrail')||'[]')||[]; }catch(e){ return []; } }
...
// v13.32 · ציד קריאת ההחלפה: (א) כל הבקשות הלא-GET האחרונות + GET עם office/user/profile בכתובת,
...
function switchHunt(){
  var out=[];
  (NET||[]).forEach(function(rec){
    var u=String(rec.url||'');
    if(/user\/ping|gw\.yad2\.co\.il\/office\/?(\?|$)|office-broker|office\/packages|account-manager|switch|select-office|profile|login|token/i.test(u))
      out.push({url:u.replace(/^https?:\/\//,'').slice(0,160),text:String(rec.text||'').slice(0,2500)});
  });
  return out.slice(-8);
}
function storageDump(){
  var out={local:[],session:[],cookies:[]};
  try{ for(var i=0;i<localStorage.length;i++){ var k=localStorage.key(i); if(/^(gm_|yad2_|ys)/.test(k))continue; var v=String(localStorage.getItem(k)||''); out.local.push({k:k,len:v.length,v:v.slice(0,200)}); } }catch(e){}
  try{ for(var j=0;j<sessionStorage.length;j++){ var k2=sessionStorage.key(j); var v2=String(sessionStorage.getItem(k2)||''); out.session.push({k:k2,len:v2.length,v:v2.slice(0,200)}); } }catch(e){}
  try{ out.cookies=String(document.cookie||'').split(';').map(function(c){ c=c.trim(); var i2=c.indexOf('='); return {k:c.slice(0,i2),v:c.slice(i2+1,i2+81),len:c.length-i2-1}; }).filter(function(c){return c.k;}); }catch(e){}
  return out;
}
function netUrlList(){
  var seen={}, out=[];
...
var exclForceArm=0;
// "לחץ שוב" תקף 30 שניות. הגדרה אחת, כדי ששני הכפתורים והבדיקות יסכימו.
function exclForceArmed(){ return !!(exclForceArm && (Date.now()-exclForceArm)<30000); }
...
function exclScanFamilyNow(force){
  ...
  if(!force && !exclDeadNow(now)){
    // 🐞 09/09 (אייל: "שאני לוחץ סרוק שוב הוא לא סורק שוב") — ההודעה הבטיחה
    //    לחיצה שנייה, אבל exclForceArm נדלק רק במסלול המנוחה. בלי הדלקה כאן
    //    הלחיצה השנייה קיבלה אותו סירוב לנצח.
    exclForceArm=now;
...
function exclScanNow(force){
  ...
  if(!force && !exclDeadNow(now)){
    exclForceArm=now;   // ראה ההערה במקבילה — ההודעה חייבת להיות מגובה בדגל
    return {ok:false,msg:'סריקה פעילה — '+leaseWhy(now)+'. לחץ שוב כדי להפעיל בכל זאת'};
  }
  if(force){gmSet('ysExclCool','0');leaseClear();}
  try{
...
// exclStallCheck הוסרה ב-v11.0. היא נועדה "לשחרר" נעילה תקועה — וזו בדיוק
// הייתה הבעיה: שומר שני שמנסה לתקן את הפרשנות של הראשון. לחכירה יש תפוגה
...
function exclSchedTick(){
  ...
  if(inQuiet(new Date(now))) skip='חלון שקט';
    ...
    skip='סריקה קודמת פעילה ('+leaseWhy(now)+')';
  }
  if(skip){
...
// מקור שני הבאגים של 01/09: שני שומרים פירשו את הצירוף אחרת, וכל אחד הניח
// שהשני משחרר. כאן: אובייקט אחד, פונקציה אחת שמחליטה, **ותפוגה מובנית** —
// ריצה לא יכולה להיחשב חיה יותר מ-25 דק' בשום צירוף, ולכן אין מה "לשחרר".
// ⏱️ מכוילים למדידה מ-08/09, לא להערכה: ריצה מלאה נמשכה 48-64 דק', ומשרד בודד
// (עמודים + הפוגות + שליפת סוכנים) לוקח דקות. הערכים הקודמים — דופק 3 דק' ותקרה
...
function leaseLive(now,l){
  l=(l===undefined)?leaseGet():l;
  if(!l||!l.start||l.end)return false;              // אין ריצה, או שהסתיימה
  if(now-(l.beat||l.start)>=LEASE_BEAT_MS)return false;  // הטאב מת
  if(now-l.start>=LEASE_MAX_MS)return false;        // תקרה קשה — זומבי פג מעצמו
  return true;
}
function leaseWhy(now){
  var l=leaseGet();
  if(!l||!l.start)return 'אין ריצה';
  if(l.end)return 'הריצה האחרונה הסתיימה לפני '+Math.round((now-l.end)/60000)+' דק׳';
  return 'דופק לפני '+Math.round((now-(l.beat||l.start))/1000)+'ש׳, רצה '+Math.round((now-l.start)/60000)+' דק׳';
}
function leaseOpen(now,kind){
  var l={id:String(now).slice(-6)+'-'+Math.random().toString(36).slice(2,6),kind:kind||'full',start:now,beat:now,end:0};
  leaseMine=l.id;
  leaseSet(l);
  if(leaseTimer)clearInterval(leaseTimer);
  leaseTimer=setInterval(function(){
    var c=leaseGet();
    if(!c||c.id!==l.id||c.end){                              // ריצה אחרת השתלטה
      leaseStop();
...
function leaseStop(){ if(leaseTimer){clearInterval(leaseTimer);leaseTimer=null;} }
// האם הטאב הזה עדיין בעל התור? אישור אייל מהשטח (09/09): "כל פעם נפתח חלון נוסף
// של סריקת משרדים". v11.4 מנע את יצירתם — אבל שום דבר לא סגר טאב שהוחלף, והוא
// המשיך לסרוק במקביל לנצח. טאב שאיבד את התור חייב לעצור ולסגור את עצמו.
function leaseOwn(){
  var l=leaseGet();
  if(!l||!l.start||l.end)return true;   // אין ריצה רשומה — אין ממי לקחת את התור
  if(!leaseMine)return true;            // לא פתחנו חכירה בעצמנו — לא חוסמים סריקה
  return l.id===leaseMine;              // עוצרים רק כשמישהו אחר באמת מחזיק אותה
}
// פירור-דרך: איפה הריצה נמצאת עכשיו. ריצה שמתה לא שולחת דיאגנוסטיקה (זו נשלחת
// רק בסיום מסודר), ולכן בלי זה "died" לא אומר איפה — וזה מה שחסר ב-19:55 (08/09).
function leaseStep(s){
  var l=leaseGet(); if(!l||l.end)return;
  l.step=String(s||'').slice(0,60); l.beat=Date.now(); leaseSet(l);
}
function leaseClose(){
  var l=leaseGet(); leaseStop();
...
function leaseReap(now){
  ...
  ledgerAdd({kind:l.kind||'full',ms:now-l.start,offices:0,ok:0,blocked:0,done:false,why:'died',step:l.step||'לא ידוע'});
  leaseClear();
  return true;
}
...
function ledgerAdd(rec){
  ...
  rec.t=Date.now();
  a.unshift(rec);
  gmSet(LEDGER_KEY,JSON.stringify(a.slice(0,20)));
  try{ledgerPost(rec);}catch(e){}
}
function ledgerPost(rec){
  var body='secret='+encodeURIComponent(SECRET)+'&action=runlog&data='+encodeURIComponent(JSON.stringify(rec));
...
// נשמרת לשם התאימות: "מותר לשגר?" = פשוט "אין חכירה חיה".
function exclScanDead(now){ return !leaseLive(now); }
// קריאה אחת שקוראת את שלוש החותמות — כדי ששום נקודת-בדיקה לא תשכח אחת מהן
function exclDeadNow(now){ return !leaseLive(now); }
...
// כל שליפה של הסורק עוברת כאן, ולכן זו הנקודה הנכונה לשני הדברים:
// (1) רענון הדופק — 09/09: הדופק התרענן רק *בין* משרדים, וסניף עם 203 מודעות
//     (עמודים + עד 15 שליפות סוכן בקצב אנושי) חורג מ-12 דק' בקלות. הריצה
//     הוכרזה מתה בזמן שעבדה, וכל הכרזה כזו פתחה טאב מתחרה.
// (2) פסק זמן — ל-fetch לא היה timeout, ובקשה תקועה הקפיאה את הריצה לנצח.
var EXCL_FETCH_MS=25000;
function exclFetchFn(){
  return function(u){
    try{leaseStep(leaseGet()&&leaseGet().step||'שולף');}catch(e){}
    ...
    return new Promise(function(res,rej){
      ...
      var timer=setTimeout(function(){
        ...
        rej(new Error('פסק זמן ('+Math.round(EXCL_FETCH_MS/1000)+'ש׳)'));
      },EXCL_FETCH_MS);
      fetch(u,opts).then(function(r){return r.text();}).then(function(t){
...
function exclRunPublic(){
  ...
  if(famOnly){
    leaseOpen(Date.now(), (typeof famOnly!=='undefined'&&famOnly)?'fam':'full');
...
// ═══ נווט-ואסוף (v13, בטא) ═══════════════════════════════════════════════
// Radware חוסם fetch ישיר לעמודי המשרד (אומת 09/09: דף "Verifying your browser").
// אבל ניווט אמיתי של הדפדפן עובר את האתגר — בדיוק כמו הזרם הפרטי, שלא נחסם
// חודשים. כאן: הטאב מנווט לעמוד /forsale?page=N של כל סניף, יד2 טוען בעצמו,
// ואנחנו קוראים את __NEXT_DATA__ מה-document הטעון (אותם פרסרים כמו ה-fetch).
// מכונת-מצבים חוצת-ניווטים: המצב ב-GM (ysNav), כל טעינה ממשיכה מהמקום.
...
function navCollectStep(){
  ...
  // act==='post' — שולחים את הסניף (אם יש שורות), ואז לסניף הבא או סיום
  var finish=function(){
    if(res.thenDone){
...
// אייל: הפאנל הוצף. הפירוט המלא נשאר בדיאגנוסטיקה, שנקראת מרחוק).
function exclRollup(summary){
  var saved=[],srverr=0,blocked=0,skipped=0,tot='',stopNote='',firstErr='';
  summary.forEach(function(e){
    e=String(e);
    if(e.indexOf('סה"כ:')===0){ tot=e; return; }
    if(/יד2 חסם|תקרת/.test(e)){ stopNote=e.slice(0,60); return; }   // סיבת עצירה — נשמרת קצר
    if(/✓/.test(e)){ saved.push(e.split(':')[0].trim()); return; }
    if(/שגיאת שרת|פסק זמן|network/.test(e)){ srverr++; if(!firstErr){var m=String(e).match(/(http=\d+[^·]*|פסק זמן[^·]*|network)/);firstErr=m?m[1].trim():'';} return; }
    if(/נחסם/.test(e)){ blocked++; return; }
    if(/דילוג|מנוחה/.test(e)){ skipped++; return; }
  });
  var parts=[];
  parts.push('נשמרו '+saved.length+(saved.length?' ('+saved.slice(0,4).join(', ')+(saved.length>4?'…':'')+')':''));
  if(srverr)parts.push(srverr+' שגיאת שרת'+(firstErr?' ('+firstErr+')':''));
  if(blocked)parts.push(blocked+' נחסמו');
  if(skipped)parts.push(skipped+' דילוג');
  if(stopNote)parts.push(stopNote);
  if(tot)parts.push(tot);
  return parts.join(' · ');
}
function exclScanOffices(OFFICES,fetchFn,dirDiag){
  // 🐞 08/09: חסימה של משרד אחד סגרה את כל הריצה (סניף 5628636 נחסם והסניף
  //    השלישי לא נסרק כלל). חסימה בודדת היא לרוב מקומית/רגעית; שלוש רצופות
  //    אומרות שיד2 סימן אותנו. blockRun = רצף, blockTot = סך הריצה.
  var i=0,summary=[],diags=[],doneThisRun=0,blocked=false,blockRun=0,blockTot=0,okTot=0,runT0=Date.now();
  var BLOCK_RUN_MAX=3;
  ...
  (function nextOffice(){
    // איבדנו את התור? ריצה אחרת השתלטה — עוצרים מיד וסוגרים את הטאב. בלי זה
    // טאב מוחלף ממשיך לסרוק במקביל, וזה מה שגרם לארבעה סורקים בו-זמנית (09/09).
    if(!leaseOwn()){
      ledgerAdd({kind:(dirDiag&&dirDiag.famOnly)?'fam':'full',ms:Date.now()-runT0,offices:OFFICES.length,ok:okTot,blocked:blockTot,done:false,why:'superseded'});
      log('🛑 ריצה אחרת השתלטה — עוצר וסוגר את הטאב');
    ...
    if(blocked || (doneThisRun>=OFFICES_PER_RUN && !famNext)){
      ...
      var rollupStop='🏢 עצירה · '+exclRollup(summary);
      gmSet('ysExclLast',new Date().toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'})+' '+rollupStop.slice(0,200));
      ledgerAdd({kind:(dirDiag&&dirDiag.famOnly)?'fam':'full',ms:Date.now()-runT0,offices:OFFICES.length,ok:okTot,blocked:blockTot,done:false,why:blocked?'blocked':'cap'});
      leaseClose();   // הריצה נגמרה
      lastScanMsg=rollupStop.slice(0,200);
    ...
    if(i>=OFFICES.length){
      ...
      var rollupDone='🏢 הושלמה · '+exclRollup(summary);
      gmSet('ysExclLast',new Date().toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'})+' '+rollupDone.slice(0,200));
      ledgerAdd({kind:(dirDiag&&dirDiag.famOnly)?'fam':'full',ms:Date.now()-runT0,offices:OFFICES.length,ok:okTot,blocked:blockTot,done:true});
      leaseClose();
      lastScanMsg=rollupDone.slice(0,200);
      try{postHB('ok');}catch(e){}
    ...
    exclCrawlOffice(office,fetchFn,function(name,part,page,n){
      ...
      if(diag&&diag.blocked){ // עמוד CAPTCHA — לא מסמנים "בוצע"
        blockRun++; blockTot++;
        summary.push(office.name+': ⛔ נחסם');
        if(blockRun>=BLOCK_RUN_MAX){
          blocked=true;
          log('⛔ '+blockRun+' חסימות רצופות — יד2 סימן אותנו, עוצר את הסריקה');
      ...
      blockRun=0;   // עמוד תקין — הרצף נשבר
...
function initPublic(){ // www.yad2.co.il — סורקים רק כשהטאב נפתח עם הדגל; גלישה רגילה לא מופרעת
  if(location.hash.indexOf(NAV_FLAG)>-1){ setTimeout(navCollectStep,2500+Math.random()*1500); return; }
