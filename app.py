...
def api_help():
    ...
    _resp = (str(j)[:240] if j is not None else "None (אין תגובה מ-Apps Script — כנראה הגרסה הפרוסה ישנה)")
...
function sendHelp(){var m=($("help_msg").value||"").trim();var st=$("help_st");if(!m){st.innerHTML="<span class=err>נא לכתוב הודעה</span>";return;}var k=$("help_kind").value;st.textContent="שולח…";api("/api/help",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:m,kind:k})}).then(function(r){if(r&&r.ok){st.innerHTML="<span style='color:#1f8a4c;font-weight:700'>✓ נשלח בהצלחה! תודה.</span>";setTimeout(closeHelp,1200);}else{st.innerHTML="<span class=err>השליחה נכשלה.</span><br><span style='font-size:10px;direction:ltr;display:block;word-break:break-all;color:#999'>"+((r&&r.resp)||"")+"</span>";}}).catch(function(){st.innerHTML="<span class=err>שגיאת רשת</span>";});}
